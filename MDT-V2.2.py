...
class MDLCreator:
    ...
    def create_file_structure_window(self):
        self.window = ctk.CTkToplevel(self.parent.window)
        ...
        ctk.CTkButton(button_frame, text="Close Folder", command=self.close_folder).pack(side="left", padx=5)
    ...
    def add_url(self):
        ...
        if url:
            indentation = "    " * len(self.current_folder)
            self.file_structure.append(f"{indentation}{url}")
    ...
    def add_folder(self):
        ...
        if folder_name:
            indentation = "    " * len(self.current_folder)
            self.file_structure.append(f"{indentation}dir [{folder_name}]{{")
            self.current_folder.append(folder_name)
            self.update_structure_display()

    def close_folder(self):
        if self.current_folder:
            indentation = "    " * (len(self.current_folder) - 1)
            self.file_structure.append(f"{indentation}}}")
    ...
    def save_mdl(self):
        ...
        filename = filedialog.asksaveasfilename(initialdir=exe_dir, defaultextension=".mdl")
        if filename:
            ...
            response = ctk.CTkMessagebox(title="Success", message="MDL file saved successfully! Do you want to start downloading this file?", icon="question", option_1="No", option_2="Yes")
            
            if response.get() == "Yes":
                self.window.destroy()
                self.parent.file_path.set(filename)
                self.parent.process_file()
...
class MDLDownloader:
    def __init__(self):
        ...
        self.file_path = ctk.StringVar(self.window)
    ...
    def descargar_archivos(self, data_lines, ruta_descarga):
        ...
        for linea in data_lines:
            ...
            if linea == "$data-end":
                break
            elif linea == "$data-start":
                continue
            elif linea.startswith("dir"):
                carpeta = linea.split("[")[1].split("]")[0]
                directorios.append(carpeta)
            elif linea == "}":
                directorios.pop()
            elif linea:  # This ensures we're not processing empty lines
                url = linea
